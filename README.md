...
- CPU は自動でツモ・捨て・パスを行い、ときどき(意図的に低い確率で)あがり宣言やポンをする
  - 配布枚数+1枚=あがり枚数という設計上、ツモ直後は「枚数だけ」なら毎回あがり宣言できてしまうため、CPU が毎ターン即あがると検証にならない。そのため `js/app.js` の `CPU_TSUMO_CHANCE` / `CPU_RON_CHANCE` / `CPU_PON_CHANCE` でわざと確率を抑えている
  - CPU のあがりは(実在する言葉かの確認をせず)自動で「成立」する。あくまで画面遷移の確認用
